...
exports.parseWatermarkId = parseWatermarkId;
...
function makeWatermark(uniqueId) {
    return `${WATERMARK_PREFIX} ${uniqueId} -->`;
}
function parseWatermarkId(commentBody) {
...
function buildSeverityTable(data) {
...
function severitySummary(checks) {
...
function buildCheckTable(checks) {
    ...
    const header = '| Severity | Check ID | Check Name | Resource | Lines |\n' +
        '|----------|----------|------------|----------|-------|\n';
    const rows = checks
        .slice()
        .sort((a, b) => {
...
function buildFileSection(file, checks) {
...
function buildComment(data, opts, uniqueId = 'default') {
    const { totals } = data;
    ...
    const metaParts = [
        ...
        totals.parsingErrors > 0
            ? `⚠️ Parsing errors: **${totals.parsingErrors}**`
            : null,
    ].filter(Boolean);
    const metaLine = metaParts.length > 0 ? `> ${metaParts.join(' · ')}` : '';
    const fileSections = [];
    const fileGroups = groupByFile(data.allFailed);
    const filteredSeverities = new Set([...opts.severities, 'UNKNOWN']);
    ...
    const coverageReport = fileSections.length > 0
    ...
    if (body.length > MAX_COMMENT_LENGTH) {
        ...
        const baseLength = [watermark, headerLine, '', summaryTable, '', metaLine].join('\n').length +
            truncationNote.length + 2;
        const keptSections = [];
        ...
        body = [watermark, headerLine, '', summaryTable, '', metaLine, '', truncatedReport, truncationNote].join('\n');
    }
    return body;
}
